...
if (
  ...
  if (priorityMatch) {
    const projectName = priorityMatch[1].trim();
    const priority = priorityMatch[2].trim();
    ...
    setMessages((prev) => [
      ...
      {
        role: "ceo",
        text: `Project "${projectName}" priority set to ${priority}.`
      }
    ]);
...
useEffect(() => {
  ...
  const handleChangeAgent = (event) => {
    ...
    const agent =
      event.detail?.agent?.trim();

    if (!projectName || !agent) return;
